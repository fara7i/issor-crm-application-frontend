...
export async function POST(request: NextRequest) {
  try {
    ...
    if (previousQuantity < quantity) {
      return badRequest(`Insufficient stock. Available: ${previousQuantity}, Requested: ${quantity}`);
    }
    ...
    await db.insert(stockHistory).values({
      ...
      type: 'REMOVE',
      reason: reason || `Removed ${quantity} units`,
      previousQuantity,
      newQuantity,
      createdBy: user.id,
    });
    ...
    return NextResponse.json({
      ...
      message: `Removed ${quantity} units from ${product[0].name}`,
    });
  } catch (error) {
